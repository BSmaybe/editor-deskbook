...
ADMIN_USER="golayoutadmin_${SUFFIX}"
ADMIN_PASS="GoLayoutPass1!"
ADMIN_EMAIL="${ADMIN_USER}@test.local"
ADMIN2_USER="golayoutadmin2_${SUFFIX}"
ADMIN2_PASS="GoLayoutPass2!"
ADMIN2_EMAIL="${ADMIN2_USER}@test.local"
...
echo ""
echo "=== Admin token and floor fixture ==="
...
if [ "$REG_STATUS" = "201" ]; then
  ...
  fail "Register admin" "HTTP ${REG_STATUS}: $(cat /tmp/go_layout_body.$$)"
fi
...
REGISTER2_BODY="{\"username\":\"${ADMIN2_USER}\",\"email\":\"${ADMIN2_EMAIL}\",\"password\":\"${ADMIN2_PASS}\",\"role\":\"admin\",\"admin_secret\":\"${ADMIN_SECRET}\"}"
REG2_STATUS="$(http_status -X POST "${PY_BASE_URL}/auth/register" -H "Content-Type: application/json" -d "$REGISTER2_BODY")"
if [ "$REG2_STATUS" = "201" ]; then
  pass "Register second admin through compatibility API"
else
  fail "Register second admin" "HTTP ${REG2_STATUS}: $(cat /tmp/go_layout_body.$$)"
fi

LOGIN2_BODY="$(curl -fsS -X POST "${PY_BASE_URL}/auth/login" \
  -H "Content-Type: application/x-www-form-urlencoded" \
  --data-urlencode "username=${ADMIN2_USER}" \
  --data-urlencode "password=${ADMIN2_PASS}")"
...
echo ""
echo "=== Floor locks ==="
LOCK_NO_AUTH_STATUS="$(http_status "${GO_BASE_URL}/floors/${FLOOR_ID}/lock")"
...
LOCK_EMPTY_BODY="$(http_body "${GO_BASE_URL}/floors/${FLOOR_ID}/lock" -H "Authorization: Bearer ${TOKEN}")"
...
ACQUIRE_LOCK_STATUS="$(http_status -X POST "${GO_BASE_URL}/floors/${FLOOR_ID}/lock" -H "Authorization: Bearer ${TOKEN}")"
ACQUIRE_LOCK_BODY="$(cat /tmp/go_layout_body.$$)"
if [ "$ACQUIRE_LOCK_STATUS" = "200" ] && echo "$ACQUIRE_LOCK_BODY" | jq -e '.floor_id == ('"$FLOOR_ID"') and .locked_by_username == "'"${ADMIN_USER}"'"' >/dev/null; then
  pass "POST /lock acquires floor lock"
else
  fail "POST /lock" "HTTP ${ACQUIRE_LOCK_STATUS}: ${ACQUIRE_LOCK_BODY}"
fi

LOCK_CONFLICT_STATUS="$(http_status -X POST "${GO_BASE_URL}/floors/${FLOOR_ID}/lock" -H "Authorization: Bearer ${TOKEN2}")"
...
LOCKED_BODY="$(http_body "${GO_BASE_URL}/floors/${FLOOR_ID}/lock" -H "Authorization: Bearer ${TOKEN2}")"
if echo "$LOCKED_BODY" | jq -e '.locked == true and .locked_by_username == "'"${ADMIN_USER}"'"' >/dev/null; then
  pass "GET /lock returns current lock owner"
else
  fail "GET /lock locked state" "$LOCKED_BODY"
fi

RELEASE_LOCK_STATUS="$(http_status -X DELETE "${GO_BASE_URL}/floors/${FLOOR_ID}/lock" -H "Authorization: Bearer ${TOKEN}")"
LOCK_RELEASED_BODY="$(http_body "${GO_BASE_URL}/floors/${FLOOR_ID}/lock" -H "Authorization: Bearer ${TOKEN}")"
...
IMPORT_NO_AUTH_STATUS="$(http_status -X POST "${GO_BASE_URL}/floors/${FLOOR_ID}/layout/import" -H "Content-Type: image/svg+xml" -d "$IMPORT_SVG")"
...
IMPORT_STATUS="$(http_status -X POST "${GO_BASE_URL}/floors/${FLOOR_ID}/layout/import" \
  -H "Authorization: Bearer ${TOKEN}" \
...
IMPORT_BAD_STATUS="$(http_status -X POST "${GO_BASE_URL}/floors/${FLOOR_ID}/layout/import" \
  -H "Authorization: Bearer ${TOKEN}" \
  -H "Content-Type: image/svg+xml" \
  -d '<html></html>')"
...
NO_AUTH_STATUS="$(http_status -X PUT "${GO_BASE_URL}/floors/${FLOOR_ID}/layout/draft" -H "Content-Type: application/json" -d "$LAYOUT_BODY")"
...
DRAFT_STATUS="$(http_status -X PUT "${GO_BASE_URL}/floors/${FLOOR_ID}/layout/draft" \
  -H "Authorization: Bearer ${TOKEN}" \
  -H "Content-Type: application/json" \
  -d "$LAYOUT_BODY")"
...
GET_DRAFT_BODY="$(http_body "${GO_BASE_URL}/floors/${FLOOR_ID}/layout" -H "Authorization: Bearer ${TOKEN}")"
...
PUBLISH_STATUS="$(http_status -X POST "${GO_BASE_URL}/floors/${FLOOR_ID}/layout/publish" -H "Authorization: Bearer ${TOKEN}")"
SVG_STATUS="$(http_status "${GO_BASE_URL}/floors/${FLOOR_ID}/layout/published.svg" -H "Authorization: Bearer ${TOKEN}")"
HTML_STATUS="$(http_status "${GO_BASE_URL}/floors/${FLOOR_ID}/layout/published.html" -H "Authorization: Bearer ${TOKEN}")"
...
SVG_BODY="$(http_body "${GO_BASE_URL}/floors/${FLOOR_ID}/layout/published.svg" -H "Authorization: Bearer ${TOKEN}")"
...
HTML_BODY="$(http_body "${GO_BASE_URL}/floors/${FLOOR_ID}/layout/published.html" -H "Authorization: Bearer ${TOKEN}")"
...
DESKS_AFTER_LAYOUT="$(http_body "${PY_BASE_URL}/desks?floor_id=${FLOOR_ID}")"
...
HISTORY_BODY="$(http_body "${GO_BASE_URL}/floors/${FLOOR_ID}/layout/history" -H "Authorization: Bearer ${TOKEN}")"
...
REVISIONS_BODY="$(http_body "${GO_BASE_URL}/floors/${FLOOR_ID}/layout/revisions?limit=20" -H "Authorization: Bearer ${TOKEN}")"
...
REVISION_BODY="$(http_body "${GO_BASE_URL}/floors/${FLOOR_ID}/layout/revisions/${REVISION_ID}" -H "Authorization: Bearer ${TOKEN}")"
...
RESTORE_STATUS="$(http_status -X POST "${GO_BASE_URL}/floors/${FLOOR_ID}/layout/revisions/${REVISION_ID}/restore" -H "Authorization: Bearer ${TOKEN}")"
